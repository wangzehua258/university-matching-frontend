...
interface UKResultData {
  id: string;
  user_id: string;
  targetCountry: string;
  recommendedSchools: UKSchool[];
  fallbackInfo?: {
    applied: boolean;
    steps: string[];
  };
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <p className="font-medium text-gray-900 text-sm">UCAS申请时间</p>
                    <p className="text-xs text-gray-600 mt-1">{result.keyInfoSummary.ucasInfo}</p>
                  </div>
                </div>
                <div className="flex items-start space-x-3">
                  <BookOpen className="w-5 h-5 text-purple-600 mt-1 flex-shrink-0" />
                  <div>
                    <p className="font-medium text-gray-900 text-sm">Foundation路线</p>
                    <p className="text-xs text-gray-600 mt-1">{result.keyInfoSummary.foundationInfo}</p>
                  </div>
                </div>
                <div className="flex items-start space-x-3">
                  <Briefcase className="w-5 h-5 text-orange-600 mt-1 flex-shrink-0" />
                  <div>
                    <p className="font-medium text-gray-900 text-sm">工作签证</p>
                    <p className="text-xs text-gray-600 mt-1">{result.keyInfoSummary.visaInfo}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-2">
                {result.applicationGuidance.steps.map((step, idx) => (
                  <div key={idx} className="flex items-start space-x-2">
                    <span className="flex-shrink-0 w-5 h-5 bg-purple-100 text-purple-600 rounded-full flex items-center justify-center text-xs font-medium mt-0.5">
                      {idx + 1}
                    </span>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {result.recommendedSchools.length === 0 ? (
              ...
              result.recommendedSchools.map((school, index) => (
                <UKSchoolCard key={school.id} school={school} rank={index + 1} />
              ))
            )}
          </div>
        </div>
...
function UKSchoolCard({ school, rank }: { school: UKSchool; rank: number }) {
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      {/* 学校头部信息 */}
